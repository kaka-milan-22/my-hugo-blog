...
- 确保所有代码示例语法正确
- 日期格式使用东八区时间 `+08:00`
- **⚠️ 重要：日期必须是当前时间或过去时间**，如果设置未来时间，Hugo 不会显示该文章
- **⚠️ 时区陷阱（踩过坑）**：GitHub Actions 构建使用的是 UTC 时间，而 front matter 用的是 `+08:00` 北京时间。**北京时间晚于当前 UTC+8 小时的值，对构建器来说仍是未来**。例如本地 12:00（北京），写 `22:00:00+08:00`（= UTC 14:00），此时构建器 UTC 04:00 会判为未来时间，文章不会出现在站点上。**推荐做法**：写当天较早的时间（如 `10:00:00+08:00` 或更早），或直接用 `date -u` 看当前 UTC 时间再反推一个安全的北京时间。
